...
export function SenderUI({ roomCode, onExit }: { roomCode: string, onExit: () => void }) {
  ...
  useEffect(() => {
    const ws = new WebSocket(`${API_BASE?.replace("http", "ws")}/ws/${roomCode}/sender`);
    wsRef.current = ws;
  ...
  const startTalking = async () => {
    ...
    processorRef.current.onaudioprocess = (e) => {
      ...
      const intensity = Math.sqrt(sum / data.length) * 100;
  ...
  return (
    ...
    >
      ...
      {/* Disconnect + Room Code UI */}
      <div className="w-full flex flex-col md:flex-row items-center justify-between gap-6 z-20 mb-4">
        <button onClick={onExit} className="order-1 md:order-last flex items-center gap-2 px-5 py-2.5 bg-red-500/10 hover:bg-red-500 border border-red-500/20 rounded-full transition-all duration-300 group shadow-lg">
          <span className="text-[10px] font-black uppercase tracking-widest text-red-500 group-hover:text-white">Disconnect Link</span>
        ...
        <div className="flex flex-col items-center gap-2 -mt-4 md:-mt-8 z-30">
          <div className="flex items-end justify-center gap-1 h-10">
            <AnimatePresence mode="wait">
              {active ? (
                <motion.div key="active-bars" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-end gap-1">
                  {[...Array(15)].map((_, i) => (
                    <motion.div
                      key={i}
          ...
          <div className="flex flex-col items-center gap-1">
            <Activity className={`text-cyan-500 transition-all duration-700 ${active ? "animate-spin scale-110 opacity-100" : "animate-pulse opacity-20"}`} size={20} />
